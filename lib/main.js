...
function resolveOrder( params ) {
	...
	for ( i = 0; i < params.length; i++ ) {
		...
		// For a ternary parameter function, we simply return the layout of the first parameter having a layout, as there is no clear rationale for picking the layout of one parameter over another and picking the first one we find is as good a rationale as any other...
		if ( p.scalar === false ) {
			break;
		}
	}
...
* @param {(ndarrayLike|*)} param3 - third PRNG parameter
...
* // returns <ndarray>
*/
setReadOnly( Random.prototype, 'generate', function generate( shape, param1, param2, param3, options ) {
	var params;
	var prng;
	var opts;
	var buf;
	...
	params = initialScan( [ param1, param2, param3 ] );
	for ( i = 0; i < params.length; i++ ) {
		dt = params[ i ].dtype;
	...
	p3 = params[ 2 ];
	prng = this._prng;
	dt = opts.dtype || ternaryOutputDataType( p1.dtype, p2.dtype, p3.dtype, this._policies.output ); // eslint-disable-line max-len
	if ( opts.order ) {
		ord = opts.order;
	} else if ( FLG ) {
	...
	if ( len === 0 ) {
		...
		if ( shape.length === 0 ) {
			...
			set( buf, 0, prng( p1.value, p2.value, p3.value ) );
			st = [ 0 ];
		} else {
	...
	ternary( [ p1.value, p2.value, p3.value, out ], prng );
	...
	*/
	function wrapper() {
		return prng( p1.value, p2.value, p3.value );
	}
});
...
* @throws {TypeError} fourth argument must be an ndarray
* @throws {TypeError} must provide valid PRNG parameters
...
setReadOnly( Random.prototype, 'assign', function assign( param1, param2, param3, out ) {
	...
	if ( !isndarrayLike( out ) ) {
		throw new TypeError( format( 'invalid argument. Fourth argument must be an ndarray-like object. Value: `%s`.', out ) );
	}
	if ( isReadOnly( out ) ) {
	...
	if ( !contains( this._odtypes, resolveStr( odt ) ) ) {
		throw new TypeError( format( 'invalid argument. %s argument must have one of the following data types: "%s". Data type: `%s`.', ORDINALS[ 3 ], join( this._odtypes, '", "' ), odt ) );
	}
